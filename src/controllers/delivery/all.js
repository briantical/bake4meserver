...
const all = ({ Delivery }, { config }) => async (req, res, next) => {
	try {
		...
		const query = { $and : [] };
		if (search) {
			query.$and.push({ $or: new Delivery().fieldsToSearch(search) });
		}
    // if need work with cords
		...
		const count = await Delivery.find(query).count();
		const businesses = await Delivery.find(query)
		//.sort({ : 1 })
			.skip(skip)
			.limit(limit);
