...
const all = ({ Flavour }, { config }) => async (req, res, next) => {
	try {
		...
		const query = { $and : [] };
		if (search) {
			query.$and.push({ $or: new Flavour().fieldsToSearch(search) });
		}
    // if need work with cords
		...
		const count = await Flavour.find(query).count();
		const businesses = await Flavour.find(query)
		//.sort({ : 1 })
			.skip(skip)
			.limit(limit);
